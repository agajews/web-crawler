...
impl IndexShard {
    pub fn open(path: &Path) -> Result<IndexShard, Box<dyn Error>> {
        ...
        Ok(Self { index, index_headers, urls, n_urls, url_headers, term_counts })
    }
    ...
    fn deserialize_url_headers(urls: &mut fs::File) -> Result<(usize, Vec<(u32, u32)>), Box<dyn Error>> {
        let mut bytes = [0; 4];
        urls.read_exact(&mut bytes)?;
